...
app.get('/selectFighter1', function(request, response) {
	db.get('SELECT account_UID FROM jsf_account WHERE username = \'' + request.session.username + '\';', function(err, results) {
		if (err) {
			console.log("ERROR:");
			console.log(err);
		 ...
		 db.all('SELECT * FROM jsf_fighters WHERE fighter_player_UID = \'' + request.session.accUID + '\';', function(err, results) {
			 request.session.fighters = results;
			 response.status(200).render('selectFighter1', {
				 	 username: request.session.username,
					 accountUID: request.session.accUID,
					 darkmode: request.session.darkmode,
					 fighters: results
...
app.post('/selectFighterN1', function(request, response) {
	db.get('SELECT account_UID FROM jsf_account WHERE username = \'' + request.session.username + '\';', function(err, results) {
		if (err) {
			console.log("ERROR:");
			console.log(err);
		 ...
		 db.all('SELECT * FROM jsf_fighters WHERE fighter_player_UID = \'' + request.session.accUID + '\' AND fighter_name = \'' + request.body.name + '\';', function(err, results) {
			 request.session.fighter1 = results;
			 response.status(200).render('selectFighter2', {
				 	 username: request.session.username,
					 accountUID: request.session.accUID,
					 darkmode: request.session.darkmode,
					 fighters: request.session.fighters,
					 fighter1: results
			 });
		 });
		}
...
app.get('/selectFighter2', function(request, response) {
	db.get('SELECT account_UID FROM jsf_account WHERE username = \'' + request.session.username + '\';', function(err, results) {
		if (err) {
			console.log("ERROR:");
			console.log(err);
		 ...
		 db.all('SELECT * FROM jsf_fighters WHERE fighter_player_UID = \'' + request.session.accUID + '\';', function(err, results) {
			 request.session.fighters = results;
			 response.status(200).render('selectFighterN2', {
				 	 username: request.session.username,
					 accountUID: request.session.accUID,
					 darkmode: request.session.darkmode,
					 fighters: results
...
app.post('/selectFighterN2', function(request, response) {
	db.get('SELECT account_UID FROM jsf_account WHERE username = \'' + request.session.username + '\';', function(err, results) {
		if (err) {
			console.log("ERROR:");
			console.log(err);
		 ...
		 db.all('SELECT * FROM jsf_fighters WHERE fighter_player_UID = \'' + request.session.accUID + '\' AND fighter_name = \'' + request.body.name + '\';', function(err, results) {
			 request.session.fighter2 = results;
			 console.log(request.session.fighter1);
			 response.status(200).render('JSFindex', {
				 	 username: request.session.username,
					 accountUID: request.session.accUID,
					 darkmode: request.session.darkmode,
					 fighters: request.session.fighters,
					 fighter1: request.session.fighter1,
					 fighter2: results
			 });
		 });
		}
...
app.get('/game', function(request, response) {
	...
	db.all('SELECT * FROM jsf_fighters WHERE fighter_player_UID = \'' + request.session.accUID + '\';', function(err, results) {
		response.status(200).render('JSFindex', {
				...
				fighters: request.session.fighters,
				fighter1: request.session.fighter1,
				fighter2: request.session.fighter2
		});
	});
...
app.get('/pongGame', function(request, response) {
	response.status(200).sendFile(path.join(__dirname + '/public/pong/index.html'));
	console.log(request.session.username + " started playing pong.");
});
